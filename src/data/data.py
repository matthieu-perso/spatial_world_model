...
# =======================================
# Grid-based Data Generation
# =======================================
# Add 3D Grid Data Generation functions
def generate_3d_grid_data(sentences, objects, spatial_map, grid_size=(9, 9, 9)):
...
def parse_sentence_to_grid(sentence, objects, spatial_map, grid_size=(9, 9, 9)):
    """Parse a sentence describing spatial relations into a 3D grid.
    ...
    # Extract objects and relation using regex
    # Basic pattern for "The X is Y the Z"
    pattern = r"The (\w+) is (.*?) the (\w+)"
    ...
    if match:
        ...
        norm_relation = normalize_relation(relation, spatial_map)
        
        # Place the reference object (obj2) at the center
        if obj2 in objects:
            grid[center_z, center_y, center_x] = objects[obj2]
        
        # Place the first object based on the relation
        if obj1 in objects and norm_relation in spatial_map:
            dx, dy, dz = spatial_map[norm_relation]
            target_x, target_y, target_z = center_x + dx, center_y + dy, center_z + dz
...
def normalize_relation(relation, spatial_map):
    """Normalize relation name to match spatial_map keys.
    
    Args:
        relation: Relation string from the sentence
        spatial_map: Dictionary of valid relation names
        
    Returns:
        Normalized relation name
    """
    # Remove "of" and "to the" for normalization
    ...
    # Return the relation if it's in spatial_map, otherwise use a default
    return relation if relation in spatial_map else "unknown"